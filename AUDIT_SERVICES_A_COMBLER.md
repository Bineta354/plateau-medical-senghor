...
|---|---|---|---|
| `paiementService` (étendu) | `Caisse.jsx` (100%, y compris `fetchDetailsJournee` — commit `0cfddf8`), `Recapitulatif.jsx` (100%) | `RechercheRapports.jsx`, `EncaissementFactures.jsx`, `SuiviCaissiers.jsx` (paiements) | 🟡 Partiel |
...
**Hors périmètre initial de cet audit, fait dans le même chantier** : `src/services/acteConsultationService.js` (nouveau) pour `facturation/Actes.jsx` — voir section 10 — et extension de `patientService` (`getByIdWithAssurance`, `getAllWithAssurance`) pour `PatientDetailsPage.jsx`/`PatientEditPage.jsx`/`Recapitulatif.jsx` — voir section 11. `Recapitulatif.jsx` est depuis le commit `e410dfb` entièrement migré (ses 2 derniers appels natifs — liste patients+assurance et `parametres_cabinet` — n'avaient aucun service dédié avant `patientService.getAllWithAssurance()` et le branchement de `parametrageService.fetchParametres()`, déjà existant mais jamais utilisé sur cette page).
...
Deux méthodes additives : `getByIdWithAssurance(id)` et `getAllWithAssurance()` ; `getById(id)`/`update(id, data)` existaient déjà et ont juste été branchés là où ils manquaient.
...
| `src/pages/caissier/Recapitulatif.jsx` (chargement initial, liste `patients`) | Peupler le sélecteur "Patient" du récapitulatif | Aucune méthode `patientService` existante ne fait cette jointure sur toute la liste (`getAll()` fait `select('*')` sans jointure, `getByIdWithAssurance()` est limité à un seul patient) → nouvelle méthode additive `getAllWithAssurance()` | `select('id, nom, prenom, assurance_id, assurances(id, nom, taux_remboursement)')` sur `patients`, tri `nom` | ✅ Migré + testé (107 patients chargés avec jointure assurance) |
| `src/pages/caissier/Recapitulatif.jsx` (chargement initial, `parametres_cabinet`) | Charger les infos du cabinet (logo, adresse, téléphone) pour l'en-tête des factures imprimées | `parametrageService.fetchParametres(tenantId)` couvrait déjà exactement ce besoin (mêmes colonnes, superset) mais n'était jamais appelé depuis cette page | `select('nom_cabinet, adresse, ville, code_postal, telephone, email, logo_url')` sur `parametres_cabinet`, `.maybeSingle()`, sans filtre tenant | ✅ Migré — `fetchParametres(userProfile?.tenant_id)`, filtre tenant ajouté au passage (amélioration multi-tenant) |
